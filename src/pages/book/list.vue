<template>
  <article class="book_content_article">
    <el-row class="warp">
...
export default {
  ...
  methods: {
    ...
    getBooks() {
      ...
      NProgress.start();
      //post方式
      api.reqGetBookListPage(params).then((data) => {
      ...
      //get方式
      // api.reqGetBookListPage1(params).then((data) => {
      //   this.total = data.total;
      //   this.books = data.books;
      //   this.listLoading = false;
      //   NProgress.done();
    ...
    delBook: function(index, row) {
      this.$confirm('确认删除该记录吗?', '提示', { type: 'warning' }).then(() => {
        ...
        NProgress.start();
        let para = { id: row.id };
        api.reqDeleteBook(para).then((res) => {
          this.listLoading = false;
          NProgress.done();
          this.$message({
            message: '删除成功',
    ...
    batchDeleteBook: function() {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        NProgress.start();
        let para = { ids: ids };
        api.reqBatchDeleteBook(para).then((res) => {
          this.listLoading = false;
          NProgress.done();
          this.$message({
            message: '删除成功',
